...
export default function InventoryManagement({ userRole }: { userRole?: string }) {
  ...
  const [_managerOutletId, setManagerOutletId] = useState('');
                      ...
                      <table className="w-full font-mono text-[11px] text-[#d4c4b0]/90">
                        ...
                        <tbody>
                          {wastageList.map((w, index) => {
                            const staffName = staffList.find(s => s.id === w.staff_id || s.employee_id === w.staff_id)?.name || w.staff_id || 'System';
                            return (
                              <tr key={w.id || index} className="border-b border-[#302117]/20 last:border-b-0 hover:bg-[#070402]/20 transition-colors">
                                <td className="py-2.5">{w.date || new Date(w.created_at).toLocaleDateString()}</td>
                                <td className="py-2.5 text-white font-serif italic text-xs font-bold">{w.item_name}</td>
                                <td className="py-2.5">{w.quantity}</td>
                                <td className="py-2.5 text-red-400">Rs. {w.value}</td>
                                <td className="py-2.5 capitalize">{w.reason}</td>
                    ...
                    <table className="w-full font-mono text-[11px] text-[#d4c4b0]/90">
                      ...
                      <tbody>
                        {stockMovementsList.map((m, index) => {
                          const staffName = staffList.find(s => s.id === m.staff_id || s.employee_id === m.staff_id)?.name || m.staff_id || 'System';
                          const isNegative = m.quantity < 0;
                          return (
                            <tr key={m.id || index} className="border-b border-[#302117]/20 last:border-b-0 hover:bg-[#070402]/20 transition-colors">
                              <td className="py-2.5">{new Date(m.timestamp).toLocaleString()}</td>
                              <td className="py-2.5 text-white font-serif italic text-xs font-bold">{m.item_name}</td>
                              <td className="py-2.5">
                                <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                                  m.type === 'sale' ? 'bg-blue-500/10 text-blue-400' :
                                  m.type === 'waste' ? 'bg-red-500/10 text-red-400' :
                                  m.type === 'addition' ? 'bg-green-500/10 text-green-400' :
                              ...
                              <td className={`py-2.5 font-bold ${isNegative ? 'text-red-400' : 'text-green-400'}`}>
                                {isNegative ? '' : '+'}{m.quantity} {m.unit || 'pcs'}
                              </td>
                              <td className="py-2.5">{staffName}</td>
                            </tr>
                          );
