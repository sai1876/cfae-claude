              ...
              {loading && orders.length === 0 ? (
                ...
                filteredOrders.map(order => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span className={`inline-block px-2 py-1 rounded text-[10px] font-mono uppercase font-bold tracking-wider ${
                        order.status === 'completed' || order.status === 'delivered' ? 'bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/30' :
              ...
              <div className="flex items-center gap-3 mb-8">
                ...
                {['pending', 'accepted', 'preparing', 'ready', 'dispatched', 'out_for_delivery'].includes(selectedOrder.status) && (
                  <button 
                    ...
                    className="mr-10 bg-[#10B981] hover:bg-[#34D399] disabled:bg-[#302117]/50 disabled:text-[#d4c4b0]/40 text-white px-4 py-2.5 rounded-xl font-mono font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all shadow-[0_0_15px_rgba(16,185,129,0.2)] cursor-pointer"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-y-3 font-mono text-xs">
                    ...
                    {selectedOrder.order_type === 'delivery' && (
                      <>
                        ...
                        <div className="text-white">{(selectedOrder.fulfillment_status === 'out_for_delivery' || selectedOrder.status === 'completed') ? formatTime(selectedOrder.updated_at || selectedOrder.created_at) : 'Awaiting Handover'}</div>
                        
                        <div className="text-[#d4c4b0]/50 uppercase">Delivered At</div>
                        <div className="text-white">{selectedOrder.status === 'completed' ? formatTime(selectedOrder.updated_at || selectedOrder.created_at) : 'Pending'}</div>
                      </>
                    )}
                    {selectedOrder.order_type !== 'delivery' && (
        ...
        {reasonModalOrder && (
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
              className="bg-[#120a06] border border-[#302117] rounded-3xl p-6 max-w-md w-full shadow-2xl relative"
            >
              <h3 className="text-xl font-serif italic text-white mb-2">Manager Override</h3>
              <p className="text-xs text-[#d4c4b0]/70 mb-4 font-mono">
              ...
              <textarea
                ...
                className="w-full bg-[#070402] border border-[#302117] rounded-xl p-3 text-sm text-white focus:outline-none focus:border-[#f8bc51] transition-colors mb-4 min-h-[100px]"
              />
              
              <div className="flex gap-3 justify-end">
                <button
                  onClick={() => setReasonModalOrder(null)}
                  className="px-4 py-2 rounded-xl text-xs font-mono font-bold text-[#d4c4b0] hover:text-white transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="bg-[#10B981] hover:bg-[#34D399] disabled:bg-[#302117]/50 disabled:text-[#d4c4b0]/40 text-white px-4 py-2 rounded-xl text-xs font-mono font-bold uppercase tracking-widest flex items-center gap-2 transition-all cursor-pointer"
                >
                  {isCompleting ? <RefreshCw size={14} className="animate-spin" /> : <CheckCircle size={14} />} 
                  Confirm
                </button>
