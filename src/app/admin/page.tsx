    ...
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (user && user.email) {
        try {
          ...
          if (!res.ok) {
            setAuthError(res.status === 403 ? 'Access Denied: Owner level required.' : 'Session verification failed.');
            setIsAuthenticated(false);
            return;
          }
          ...
          setAuthError('Network error checking session.');
          setIsAuthenticated(false);
        }
      } else {
    ...
    if (authError) {
      return (
        <div className="h-screen w-full max-w-full bg-[#060403] text-[#f7dec4] flex items-center justify-center font-sans overflow-hidden">
          <div className="flex flex-col items-center gap-6 bg-[#120a06]/80 p-10 rounded-3xl border border-red-500/30">
            <div className="w-16 h-16 bg-red-500/20 text-red-500 rounded-full flex items-center justify-center">
              <ShieldCheck size={32} />
            </div>
            <div className="text-center">
              <h2 className="font-serif text-2xl text-red-400 mb-2">Access Denied</h2>
              <p className="font-mono text-xs text-[#d4c4b0]/70 uppercase tracking-widest">{authError}</p>
            </div>
            <div className="flex gap-4 mt-4">
              <button onClick={() => window.location.href = '/manager'} className="px-6 py-2 bg-[#302117] hover:bg-[#302117]/80 text-[#f8bc51] border border-[#f8bc51]/40 rounded-lg font-mono text-xs uppercase tracking-widest transition-colors">
                Go to Manager Portal
              </button>
              <button onClick={handleLogout} className="px-6 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/40 rounded-lg font-mono text-xs uppercase tracking-widest transition-colors">
                Sign Out
              </button>
            </div>
          </div>
