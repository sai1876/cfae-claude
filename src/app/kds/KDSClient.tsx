...
export default function KDSClient({ role, staffDetails }: KDSClientProps) {
  ...
  useEffect(() => {
    ...
    console.log(`[KDS] Listening to collection: 'orders', Filter: created_at >= ${timeLimit}`);
    const q = query(
  ...
  const toggleItemReady = async (orderId: string, itemId: string, currentStatus: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409) {
          alert('Order changed, refresh KDS');
        } else {
          const json = await res.json();
          alert(json.error || 'Failed to update item status');
        }
        return;
      }

      // Recalculate top-level status via API
  ...
  // Bump station ticket via API sequentially
  const handleBumpTicket = async (orderId: string) => {
    ...
    try {
      const updatedItems = [...rawOrder.items];

      for (let i = 0; i < rawOrder.items.length; i++) {
        const item = rawOrder.items[i];
        if (isAllowedItem(item) && item.item_status !== 'ready') {
          const res = await fetch('/api/orders/update-kds-item-status', {
            method: 'POST',
            headers: { 
              'Content-Type': 'application/json',
          ...
          if (!res.ok) {
            if (res.status === 409) throw new Error('Order changed, refresh KDS');
            const json = await res.json();
            throw new Error(json.error || 'Failed to bump item');
          }
          updatedItems[i] = { ...item, item_status: 'ready' as const };
        }
      }
      ...
      if (e.message === 'Order changed, refresh KDS') {
        alert(e.message);
      }
    }
  };
        ...
        <AnimatePresence mode="popLayout">
          {filteredOrders.length === 0 ? (
            ...
            filteredOrders.map(order => {
              const canBumpTicket = true; // Allow bumping at any time to mark all as ready
              ...
              return (
                ...
                >
                  ...
                  <div className={`p-5 border-b flex justify-between items-start ${
                    isUrgent ? 'bg-[#e8621a]/10 border-[#e8621a]/30' : 'bg-[#120a06]/50 border-[#302117]'
                  }`}>
                    <div>
                    ...
                    <div className={`text-right font-mono ${isUrgent ? 'text-[#e8621a] animate-pulse' : 'text-[#d4c4b0]/70'}`}>
                      ...
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl flex items-center gap-4 transition-all cursor-pointer ${
                          item.item_status === 'preparing' 
                            ? 'bg-[#10B981]/10 border border-[#10B981]/20 opacity-90' 
                            : 'bg-[#120a06] border border-[#302117] hover:border-[#f8bc51]/50'
                        }`}
                      >
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${
                          item.item_status === 'preparing' ? 'bg-[#10B981] text-white' : 'bg-[#302117] text-[#f8bc51]'
                        }`}>
                          {item.item_status === 'preparing' ? <Utensils size={16} /> : `${item.quantity}x`}
                        </div>
                        <div className="flex-1">
                          <p className={`font-bold leading-tight ${item.item_status === 'preparing' ? 'text-[#10B981]' : 'text-white text-lg'}`}>
                            {item.name}
                          </p>
