...
export default function KDSStationPage({ params }: { params: { station: string } }) {
  ...
  useEffect(() => {
    ...
    setOrders(mockActiveOrders as unknown as OrderDocument[]);
  }, [station]);
      ...
      if (order.order_id === orderId) {
        return {
          ...
          items: order.items.map(item => 
            item.station === station.toUpperCase() ? { ...item, item_status: 'ready' as const } : item
          )
        };
      }
      return order;
    }).filter(order => order.items.some(item => item.station === station.toUpperCase() && item.item_status === 'ordered')));
  };
  ...
  const activeStationOrders = orders.filter(order => 
    order.items.some(item => item.station === station.toUpperCase() && item.item_status === 'ordered')
  );
