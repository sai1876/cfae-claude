...
import { Minus, Plus, Trash2, ArrowRight, Tag, Info, RotateCw, MapPin, CheckCircle2, AlertCircle, X,  Sparkles } from 'lucide-react';
import { useStore } from '@/store/useStore';
import { db, auth } from '@/lib/firebase';
import { collection, query, where, getDocs } from 'firebase/firestore';
import { signInWithCustomToken } from 'firebase/auth';
import { updateUserProfile, fetchOutlets, fetchOffers, fetchMenuItems } from '@/lib/dbService';
...
export default function CartPage() {
  ...
  const { cart, setCart, addToCart, removeFromCart, updateQuantity, clearCart, user, userProfile, customerOutlet, setCustomerOutlet } = useStore();
  ...
  // Totals calculation using shared helper
  const {
    subtotal,
    promoDiscount,
    pointsRedeemed,
    ...
    try {
      ...
      const response = await fetch('/api/orders/create', {
        ...
        body: JSON.stringify({
          ...
          hatch: orderType === 'pickup' ? selectedHatch : undefined,
          tableNo: orderType === 'dine-in' ? tableNo : undefined,
          outlet: customerOutlet,
      ...
      try {
        ...
        throw new Error('Server returned an invalid response. Please try again or contact staff.');
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--background)', paddingBottom: 100 }}>
      ...
      <div style={{ padding: 16 }}>
          ...
          {(['dine-in', 'pickup', 'delivery'] as const).map(type => {
            ...
            return (
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-[13px] font-bold capitalize transition-all ${
                  isSelected 
        ...
        {/* Bill Summary */}
        <div className="bg-white rounded-2xl p-5 mb-8 border border-border shadow-sm">
          <h3 className="text-foreground text-sm font-black uppercase tracking-wider mb-4">Bill Summary</h3>
          <div className="flex flex-col gap-3">
            ...
            {promoDiscount > 0 && (
              <div className="flex justify-between text-[#10B981] text-sm font-semibold">
            ...
            <div className="flex justify-between text-muted-foreground text-sm">
              <span className="flex items-center gap-1">Platform Fee <Info size={12} className="opacity-50" /></span>
              <span>₹{platformFee.toFixed(2)}</span>
            </div>
            <div className="h-px bg-border my-2" />
            <div className="flex justify-between items-center">
              <span className="text-foreground text-base font-black">To Pay</span>
              <span className="text-foreground text-xl font-black">₹{Math.round(total)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Fixed Bottom CTA */}
      <div className="fixed bottom-[80px] left-0 right-0 bg-white border-t border-border/50 p-4 pb-4 z-50 shadow-[0_-4px_16px_rgba(0,0,0,0.05)]">
        <button
          onClick={handlePlaceOrder}
          disabled={isPlacingOrder}
          className="w-full bg-primary text-primary-foreground border-none py-4 px-4 rounded-xl flex items-center justify-between active:scale-[0.98] transition-transform disabled:opacity-70 disabled:cursor-wait"
        >
          <div className="flex flex-col items-start">
            <span className="text-xs font-bold opacity-90 tracking-wide uppercase">To Pay</span>
            <span className="text-lg font-black leading-none mt-0.5">₹{Math.round(total)}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-bold text-[15px] uppercase tracking-wide">{isPlacingOrder ? 'Processing...' : user ? 'Place Order' : 'Login to Continue'}</span>
