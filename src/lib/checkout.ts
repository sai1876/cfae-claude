  ...
  if (userSnap.exists()) {
    ...
    if (totalOrders <= KICKBACK_TIERS.TIER_1.maxOrders) {
      ...
      if (userData.referred_by) {
        ...
        if (querySnapshot.empty && userData.referred_by.startsWith("HAU HAU_")) {
          const fallbackCode = "Hau Hau_" + userData.referred_by.substring(8);
          q = query(collection(db, 'users'), where("referral_code", "==", fallbackCode));
          querySnapshot = await getDocs(q);
        } else if (querySnapshot.empty && userData.referred_by.startsWith("HAUHAU_")) {
          const fallbackCode = "Hau Hau_" + userData.referred_by.substring(7);
          q = query(collection(db, 'users'), where("referral_code", "==", fallbackCode));
          querySnapshot = await getDocs(q);
        }
